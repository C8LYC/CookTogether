...
public class Sink : Block
{
    ...
    public override void Interact(Player player)
    {
        switch (state)
        {
            case WashingState.Idle:
                ...
                else if (isWashingPaused)
                {
                    // If player interacts without holding anything, allow them to move
    ...
    {
        // Start from saved progress or from beginning
        washTimer = isWashingPaused ? savedProgress : 0f;
        
        while (washTimer < washTime)
        {
            washTimer += Time.deltaTime;
            float progress = Mathf.Clamp01(washTimer / washTime);
            // Could update washing progress visualization here
            yield return null;
        }
